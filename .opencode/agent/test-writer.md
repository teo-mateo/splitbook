...
- **NEVER edit files under `tests/SplitBook.Api.Tests/Infrastructure/`.** That's shared test infrastructure (`AppFactory.cs`, `TestOnlyDtos.cs`, etc.) used by every test class. You are only allowed to create/modify test files under `tests/SplitBook.Api.Tests/Features/<YourFeature>/`. If the infrastructure looks wrong or incomplete for your criterion, STOP and delegate the question to `@researcher` — do not "clean up" the shared fixture. If you see redundant-looking code in `Infrastructure/`, assume it's load-bearing until proven otherwise.
...
- After writing, run `dotnet build` — compile must succeed. Then run **TWO** test passes:
  1. **Full suite:** `dotnet test` with NO filter. ALL pre-existing tests must still pass. If any green test went red because of your changes, revert your changes and either retry differently or return with an explicit failure report ("my changes broke pre-existing tests, aborting").
  2. **Filtered:** `dotnet test --filter "FullyQualifiedName~<YourNewTestName>"` — your single new test must fail for the RIGHT reason (endpoint missing → 404, method returning wrong status, assertion on absent behavior). **A crash in the test fixture or an exception in setup/teardown is NOT a valid red.** If your "red" is a fixture exception, something is wrong with your test OR with your changes to shared code (see rule above) — investigate, don't declare it red.
- Return a short report: the test file path + test name, the expected red reason, the full-suite pass count confirming no regressions, and the filtered `dotnet test` failure excerpt. If the full suite has any regression, the report must be a failure report instead — don't pretend red.
