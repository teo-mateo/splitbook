...
### L-FE12: Wrap atomic values in their own elements so RTL can query them
- **Observed in:** slice 3 (Groups — Detail)
- **Lesson:** When a component renders composite text (e.g. "Paid by Alex · €60.00 · 2 participants"), each atomic value must be in its own element (`<span>€60.00</span>`) so `screen.findByText()` can find it. Also, when the same text string appears in multiple places on the page (e.g. a member's name in both the Members list and an ExpenseItem), use `findAllByText` and assert the count, or scope the query to a container. Do not fight RTL by making the component render monolithic text blobs just to make a test pass.
- **Why:** Every future screen that renders lists of items with embedded values (expense feed, settlement list, balance display, profile summary) will need to assert on specific values within composite text. This is a structural requirement of RTL's text-matching strategy, not a test quirk. Per L-H11, the first component that establishes this pattern sets the precedent for all later slices.
