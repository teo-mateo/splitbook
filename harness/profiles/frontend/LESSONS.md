...
### L-FE10: Zod `z.string().datetime()` rejects .NET DateTimeOffset wire format — use `z.string()` for datetime fields
- **Observed in:** slice 2.1 (Auth datetime hotfix)
- **Lesson:** When the backend is .NET, datetime fields in API responses use `DateTimeOffset` wire format (e.g. `2026-05-17T16:28:52.6134551+00:00`), which has 7-digit fractional seconds and a timezone offset. Zod's `z.string().datetime()` rejects this — even `z.string().datetime({ offset: true })` is too strict for the subsecond precision. Use `z.string()` for datetime fields in response schemas. Add a regression test that parses the literal backend datetime string to catch drift.
- **Why:** This is a cross-cutting interop issue. Every future slice that adds datetime fields (expense dates, settlement dates, group creation timestamps) will hit it again. The fix is permissive by design — the frontend doesn't need to validate datetime format; it just needs the string to pass through to `Intl` or `Date` constructors.
...
### L-FE12: Wrap atomic values in their own elements; use `within()` to scope queries
- **Observed in:** slice 3 (Groups — Detail); reinforced slice 4 (Add/Remove member)
- **Lesson:** When a component renders composite text (e.g. "Paid by Alex · €60.00 · 2 participants"), each atomic value must be in its own element (`<span>€60.00</span>`) so `screen.findByText()` can find it. When the same text string appears in multiple places (e.g. a member's name in both a Members list and a dialog), scope the query to a container using `within(container).getByRole(...)` — never fall back to `container.querySelector()`, `container.querySelectorAll()`, or `.closest()` chains. Do not fight RTL by rendering monolithic text blobs.
- **Why:** Every future screen that renders lists of items with embedded values (expense feed, settlement list, balance display, profile summary) will need to assert on specific values within composite text. This is a structural requirement of RTL's text-matching strategy, not a test quirk. Per L-H11, the first component that establishes this pattern sets the precedent for all later slices. Slice 4 confirmed the antipattern (raw DOM traversal) reappears when unchecked.

### L-FE13: `element.closest()` returns `Element | null`, not `HTMLElement` — cast explicitly under strict mode
- **Observed in:** slice 5 (Groups — Archive) — tests used `element.closest('selector')!` to find a container for `within()` scoping. Compiled fine under `vitest run` but failed `pnpm build` (`tsc -b`) because `closest()` returns `Element | null` and RTL's `within()` expects `HTMLElement`.
- **Lesson:** When you must use `closest()` to scope a RTL query, cast the result: `within(element.closest('selector') as HTMLElement).getByRole(...)`. The non-null assert (`!`) only removes `null`, not the `Element` → `HTMLElement` widening. Prefer avoiding `closest()` entirely (L-FE12), but when it is the right tool, the cast is required under `strict: true`.
- **Why:** `vitest run` does not run `tsc -b`, so this passes green and only surfaces at the DoD build gate — the worst timing. Applies to any DOM method that returns `Element` where the downstream API expects `HTMLElement` (also `element.parentElement` in strict contexts).
