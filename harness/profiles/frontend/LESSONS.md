...
### L-FE11 [HUMAN]: Router-param components MUST be tested through `<Routes><Route path>` — `MemoryRouter` alone gives empty `useParams()`
- **Observed in:** slice 3 (Groups — Detail) — the test-writer spent ~65 min and ~69k output tokens spiralling on criterion 2. `GroupDetail` reads its id via `useParams()`, but the test rendered it in a bare `<MemoryRouter initialEntries={['/groups/<id>']}>` with **no** `<Routes><Route path="/groups/:id">`. React Router only populates `useParams()` when a matching `<Route>` is mounted; without one, `useParams()` returns `{}`, so `id` was `undefined`, the component fetched `GET /groups/undefined`, the response failed schema parsing, the screen showed the error state, and the dependent balances request never fired — making the test impossible to turn green. The model misread this as an "MSW handler ordering" bug and never connected the literal `/groups/undefined` in its own vitest output to the missing route.
- **Lesson:** Any component that calls `useParams()` / `useSearchParams()` / `useLocation()` MUST be rendered in tests through an actual route match: `<MemoryRouter initialEntries={['/groups/' + id]}><Routes><Route path="/groups/:id" element={<GroupDetail/>}/></Routes></MemoryRouter>`. `MemoryRouter` `initialEntries` only sets the URL; it does NOT fill route params. When a test fails and the symptom is "component is in its error/empty state", check `useParams()` resolves BEFORE blaming mocks/MSW. A literal `undefined` in a mocked request URL is the signature of this bug.
- **Why:** This is a propagating test-harness pattern bug, not a one-off. Per L-H11 (mirror the nearest sibling), the first router-param test's scaffold is copied verbatim into every later slice. Every upcoming param-route screen — Groups Add/Remove member, Expenses Add/Exact/Edit (`/groups/:id/expenses/:id/edit`), Settlements — will hit the identical trap and spiral identically unless the route-wrapped render pattern is the established precedent from its first use. Fixing one slice is insufficient; the pattern must be the norm.

### L-FE14: Primary must verify test-writer output matches the assigned criterion before accepting green
- **Observed in:** slice 5 (Groups — Archive) — test-writer was invoked for criterion "5xx error toast" but the test body was merged into an existing 204 test. The test name read "on 204 No Content…" while the MSW handler returned 204 and assertions checked for an error message. The 5xx test was missing entirely. The primary accepted the output without checking that the test name matched the criterion.
- **Lesson:** After `@test-writer` returns, the primary must check: (1) the test name contains the criterion's key phrase, (2) the MSW handler returns the status code the criterion requires, (3) the assertions test the behavior the criterion describes. If any mismatch, reject and re-invoke. Do not trust a green filtered run alone — a test can pass green while testing the wrong thing.
- **Why:** A test that passes green while testing the wrong criterion is worse than no test: it gives false confidence and the actual failure mode goes uncovered until a human reviews or a later slice exposes the gap. This is a primary-side gate, not a test-writer constraint.

### L-FE15: TanStack Query cache pollutes sibling tests — use unique IDs per test when mounting real `<App>`
- **Observed in:** slice 6 (Expenses — Add) — `ExpenseForm.test.tsx` mounted `<App />` (with a singleton `QueryClient`) for every test. Tests using the same group ID received stale cached data from previous tests, causing intermittent failures. Fix: each test uses a unique group UUID so MSW handlers and Query cache keys don't collide.
- **Lesson:** When tests mount the real `<App>` (which includes a shared `QueryClient` provider), every test must use unique resource IDs (group IDs, expense IDs, etc.) so TanStack Query cache keys are isolated per test. Reusing IDs across tests causes one test's cached data to leak into the next test's assertions. If tests must share an ID, wrap the render in `QueryClientProvider` with a fresh client, or call `queryClient.clear()` in `afterEach`.
- **Why:** Every future slice that tests param-route screens by mounting `<App>` (expenses edit, settlements, profile) will share this singleton cache. Without per-test ID isolation, tests become order-dependent and flaky — the hardest bug to debug because they pass in isolation but fail in suite order.

### L-FE16: Do not put side effects in `useState` initializers
- **Observed in:** slice 6 (Expenses — Add) — the primary wrote `useState(() => { /* auto-check payer checkbox */ })` to perform a side effect (modifying form state) during initialization. This is a React anti-pattern: the `useState` initializer runs during render and must be pure. The broken code was removed because the `onChange` handler already handled the logic.
- **Lesson:** Never use `useState`'s lazy initializer for side effects. Side effects belong in `useEffect` (or, preferably, in event handlers/callbacks where causality is explicit). The model confuses the lazy initializer's function syntax with `useEffect`'s dependency-free form. If you find yourself writing `useState(() => { someAction() })`, stop and ask whether the action should be in an event handler or a `useEffect`.
- **Why:** This model routinely conflates `useState` lazy init with `useEffect`. The pattern will recur in any slice with auto-populated form fields, derived state, or initialization logic. React 18's strict mode doubles server-side effects in dev, making the bug visible but confusing; in production it silently runs at the wrong time.

### L-FE17: Shared form primitives must use `forwardRef` to work with React Hook Form's `register()`
- **Observed in:** slice 6 (Expenses — Add) — `Input`, `Select`, `CurrencyInput`, and `DateInput` were scaffolded as empty stubs without `forwardRef`. When the primary implemented them, React Hook Form's `register()` could not attach DOM refs because the components were plain functions that didn't forward refs. Adding `forwardRef` to each component fixed the integration.
- **Lesson:** Any shared input component that wraps a native `<input>`, `<select>`, or `<textarea>` and is used with React Hook Form must be defined with `forwardRef`. The `ref` prop is how RHF attaches DOM references for value reading, focus management, and validation. When scaffolding shared form primitives (per L-H2), include `forwardRef` in the stub signature so the component works with RHF from the first test.
- **Why:** This is a structural requirement of the RHF integration pattern. Every future slice that introduces new form fields (settlements, profile edits) will use these shared primitives. If the stub lacks `forwardRef`, the first test that mounts the form with RHF will fail with a confusing "ref not forwarded" error, and the model will waste time debugging what looks like an RHF misconfiguration.

### L-FE18: Adding query-firing children to shared parents pollutes sibling test files
- **Observed in:** slice 8 (Expenses: List + Pagination) — `ExpenseList` was added to `GroupDetail`, firing a TanStack Query with `skip=0&take=10`. Existing `ExpenseForm.test.tsx` tests that mount real `<App>` and navigate back to `GroupDetail` now produce stderr noise (unmocked fetch errors) because those tests don't mock the new `ExpenseList` query. Tests still pass (error boundary absorbs it) but the noise masks real issues.
- **Lesson:** After adding a query-firing component to a shared parent (like `GroupDetail`), run the **full** `pnpm exec vitest run` and check stderr for unmocked query noise in **sibling** test files — not just the current slice's tests. If sibling tests mount `<App>` and reach the parent, they need MSW handlers for the new query. Add the handlers to those sibling files, or the noise accumulates and masks genuine failures.
- **Why:** The model focuses on its own test file turning green and never checks sibling files. This is a silent degradation: tests pass but stderr fills with unmocked errors that hide real problems. Every future slice that adds a new section to `GroupDetail` (settlements list, balances, etc.) will repeat this. Running the full suite and checking stderr catches it; the filtered run does not.
