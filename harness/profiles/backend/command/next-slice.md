...
**Git is forbidden in this run.** The autopilot (an out-of-band controller) owns all git operations. Never invoke `git` directly — the permission system will reject it. The reviewer agent reads the slice diff from `harness/logs/runs/slice-context/` (staged by the autopilot), not from `git diff`. If you need to know what the previous commit was, read `harness/logs/runs/slice-context/last-commit.txt`.
