...
**You have NO access to git.** The autopilot (an out-of-band controller) owns all git operations. Do not invoke `git` — the permission system will reject it. Use the filesystem directly.

Hard rules:

1. **Never invoke `git`.** All git permissions are denied. The autopilot has prepared the slice diff context for you at `harness/logs/runs/slice-context/` (files: `files.txt`, `diff.txt`, `last-commit.txt`). Read those instead.
2. **Read the slice from the working tree.** The slice's modified + untracked files are listed in `harness/logs/runs/slice-context/files.txt` (one per line, prefixed `M ` or `?? `). For modified files, `harness/logs/runs/slice-context/diff.txt` shows the unified diff. For untracked files, `Read` them directly.
3. If `harness/logs/runs/slice-context/files.txt` is missing or empty, report `fail` with finding "no slice context provided — autopilot did not stage the diff."

## Before you speak

1. `cat harness/logs/runs/slice-context/files.txt` to see the slice's surface area (modified + untracked files).
2. `cat harness/logs/runs/slice-context/diff.txt` to see modifications against the last commit.
3. For each `??` (untracked) file, read it directly with the Read tool.
4. `cat harness/logs/runs/slice-context/last-commit.txt` for the prior commit's subject (just for context).
...
1. The slice is uncommitted; you also have no git access. Look at file mtimes with `stat` or `ls -la` on the untracked production files vs test files — production file mtimes **later** than test file mtimes is consistent with L-H2. Production file mtimes much earlier than test file mtimes is a violation.
...
- [ ] **L-H2 verified via file mtimes / tool-call order** (explicit — cite the evidence you checked)
