...
- Call multiple tools in one response when they are independent — e.g. parallel reads of related files. Dependent calls must be sequential.
- **Never invoke `git`.** The autopilot owns all git ops; permissions deny git outright. For knowledge of changed files use `harness/logs/runs/slice-context/files.txt` and `diff.txt`.
