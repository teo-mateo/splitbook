...
### Groups
- `POST /groups` — `{name, currency}` → `201 GroupDto` (`{id, name, currency, memberCount}`).
- `GET /groups` — list groups the caller belongs to → `200 [GroupDto]` (`{id, name, currency, memberCount, netAmountMinor}`).
...
- `POST /groups/{id}/archive` → `204` (always succeeds; archive is the escape hatch for groups with outstanding balances — see D-06).
